...
N> The “Save” option in the toolbar, will not be enabled if the file is not edited, or if the file is loaded using `Stream` and `PdfLoadedDocument` objects. In these cases, you can use the “Save As” option.
...
The following keyboard shortcuts can be used to save the files, when toolbar is enabled.
* `Ctrl + S` - If the “Save” option is enabled in PDF Viewer, it performs the “Save” in the PDF document. Else, it performs “Save As” in the PDF document.
