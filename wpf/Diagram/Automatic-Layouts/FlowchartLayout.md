...
|![WPF Diagram displays ManualInput Symbol](Automatic-Layouts_images/wpf-diagram-manualinput-symbol.png)|ManualInput|Represents the manual input of data into a field or step in a process. Use when the value is keyed in by a user. (File renamed to `…-symbol.png` to match the other entries.)|
