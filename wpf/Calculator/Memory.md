...
# Memory in WPF Calculator

The [WPF Calculator](https://help.syncfusion.com/cr/wpf/Syncfusion.Windows.Controls.Input.SfCalculator.html) control supports memory operations that let users store, recall, add to, subtract from, and clear a single value, much like a standard pocket calculator. The memory is held per `WPF Calculator` instance and is reset to `0` when the control is first created.
...
The following buttons are available in the WPF Calculator control to perform memory-related operations.

## MS

The `MS` button performs memory storage. Clicking the `MS` button will store the current WPF Calculator value in memory, where it can be retrieved and used later.
...
Clicking the `M+` button will add the current WPF Calculator value to the value already stored in memory. The new total is then stored in memory.

## M-

Clicking the `M-` button will subtract the current WPF Calculator value from the value already stored in memory. The new total is then stored in memory.
