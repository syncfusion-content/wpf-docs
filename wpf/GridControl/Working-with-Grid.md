...
> _Note: SetHidden method accepts the following three parameters:_ 
...
* _Third is a boolean parameter that determines whether to hide or unhide the specified number of rows or columns. The rows/columns will be hidden when this parameter is set to true._
...
> _Note: To prevent resizing of specific row or column, it is required to handle ResizingRows and ResizingColumns events._
...
> _Note: You can track the moment the rows or columns are inserted by handling the RowsInserted and ColumnsInserted events._
...
> _Note: You can track the moment the rows or columns are moved by handling the RowsMoved and  ColumnsMoved event._
...
> _Note: Setting the Flag to None will disable selecting of cells._
...
> _Note: Record can be selected using a single mouse click or using UP or DOWN Arrow Keys_
...
> _Note: It does not support the use of SHIFT, CTRL and arrow keys to extend the selection._
...
> _Note: You can also provide numeric values for *._ 

> _* can be preceded by any double value._
