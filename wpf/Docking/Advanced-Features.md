...
## Setting Visual Styles for WPF Docking Control
...
In the above image, the WPF Tabbed MDI Forms is removed from the WPF Docking Control. When the TDI or MDI child elements are removed, no client area is available for displaying other content. To define a client area, use a UI element called `Client Control` and place the required content within it. The content is then displayed in the client area of the WPF Docking control.
