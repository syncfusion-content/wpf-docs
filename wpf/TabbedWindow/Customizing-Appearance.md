...
Use `HeaderTemplate` and `ContentTemplate` to present icons, badges, editable headers, and rich content inside tabs. Keep header templates lightweight to avoid layout overhead when many tabs are open.
...
## Styling the new‑tab button (NewTabButtonTemplate)

Control visibility and appearance of the new‑tab button using `EnableNewButton` and `NewTabButtonTemplate`. Provide accessible content and keyboard focus visuals for the button.
