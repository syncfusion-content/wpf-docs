...
Show or hide per‑item close affordance using `SfTabItem.CloseButtonVisibility` (bool). Close a tab by removing its `SfTabItem` from `Items`.
...
Enable the new‑tab button with `SfTabControl.EnableNewButton` and customize it with `NewTabButtonTemplate`. Handle `AddingNewTab` to supply default content.

{% tabs %}

{% highlight C# %}

tabControl.EnableNewButton = true;

tabControl.AddingNewTab += TabControl_AddingNewTab;

private void TabControl_AddingNewTab(object sender, AddingNewTabEventArgs e)
{
