...
Here, the `WPF PropertyGrid` hides the properties `AccountNumber`, `Bank` and `Email` properties which are specified in the `HidePropertiesCollection`.
...
Here, the `WPF PropertyGrid` does not display the `AccountNumber`, `Bank` and `Email` properties whose `Browsable` and `Bindable` attribute values are `false`.
...
Here, the `WPF PropertyGrid` does not display the `Bank` and `AccountNumber` properties. The `Bank` and `AccountNumber` properties are not displayed because the `Browsable` and `Bindable` attribute values are `true` while the `AutoGeneratedField` of `DisplayAttribute` is `false`. For the `ID` property, the `Browsable` is `false`, so the `AutoGeneratedField` property has no effect.
...
Here, the `Experience` and `Name` properties are not displayed in the `WPF PropertyGrid`, since the properties was restricted to be added in `WPF PropertyGrid` by the `AutoGeneratingPropertyGridItem` event.
...
Here, The SearchBox is hidden in the `WPF PropertyGrid`.
