...
We can decide the properties that need to be displayed in the [WPF PropertyGrid](https://www.syncfusion.com/wpf-controls/propertygrid) by hiding the unwanted properties using collections and attributes. We can navigate to a particular property by using the default SearchBox.

## Hide the Properties using Collection

We can hide the properties using the [HidePropertiesCollection](https://help.syncfusion.com/cr/wpf/Syncfusion.Windows.PropertyGrid.PropertyGrid.html#Syncfusion_Windows_PropertyGrid_PropertyGrid_HidePropertiesCollection) property. It is used to hide the mentioned properties which are already present in the [SelectedObject](https://help.syncfusion.com/cr/wpf/Syncfusion.Windows.PropertyGrid.PropertyGrid.html#Syncfusion_Windows_PropertyGrid_PropertyGrid_SelectedObject). Properties can also be hidden at runtime using `HidePropertiesCollection`.
...
Here, the `PropertyGrid` hides the `AccountNumber`, `Bank`, and `Email` properties, which are specified in the `HidePropertiesCollection`.

N> The `HidePropertiesCollection` cannot hide the properties which are added using `DynamicDescriptor`.
...
Here, the `PropertyGrid` does not display the `AccountNumber`, `Bank`, and `Email` properties whose `Browsable` and `Bindable` attribute values are `false`.

N> If we use both the `Browsable` and `Bindable` attributes, the `Browsable` attribute has a higher priority.

### Hide the Properties using Display.AutoGeneratedField

If a property has the value of the `Browsable` attribute as `true` and the `Bindable` attribute as `true`, then the property is not hidden. We can hide the property by setting the [AutoGeneratedField](https://docs.microsoft.com/en-us/dotnet/api/system.componentmodel.dataannotations.displayattribute.autogeneratefield?view=netframework-4.8#System_ComponentModel_DataAnnotations_DisplayAttribute_AutoGenerateField) of `Display` to `false`. The value of `AutoGeneratedField` has no effect when the property is marked `Browsable` or `Bindable` as `false`.
...
Here, the `PropertyGrid` does not display the `Bank` and `AccountNumber` properties. The `Bank` and `AccountNumber` properties are not displayed because the `Browsable` and `Bindable` attribute values are `true` while the `AutoGeneratedField` of `DisplayAttribute` is `false`. For the `ID` property, `Browsable` is `false`, so the `AutoGeneratedField` property has no effect.
...
Here, the `Experience` and `Name` properties are not displayed in the `PropertyGrid` because their generation was restricted through the `AutoGeneratingPropertyGridItem` event.

Click [here](https://github.com/SyncfusionExamples/wpf-property-grid-examples/tree/master/Samples/AutoGeneratingPropertyGridItem%20event) to download the sample that showcases the property filtering support using the `AutoGeneratingPropertyGridItem` event.

## Searching the Properties

If the `PropertyGrid.SelectedObject` contains many properties, it is difficult to find individual properties and nested properties. Now, you can easily find the required properties by searching the property name in the SearchBox. The SearchBox will filter and display the properties that contain the searched text. The SearchBox is shown by default, and you can hide it by setting the [SearchBoxVisibility](https://help.syncfusion.com/cr/wpf/Syncfusion.Windows.PropertyGrid.PropertyGrid.html#Syncfusion_Windows_PropertyGrid_PropertyGrid_SearchBoxVisibility) property to `Collapsed`.
