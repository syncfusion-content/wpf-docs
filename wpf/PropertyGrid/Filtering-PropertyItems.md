...
Here, the `PropertyGrid` does not display the `Bank` and `AccountNumber` properties. The `Bank` and `AccountNumber` properties are not displayed because the `Browsable` and `Bindable` attribute values are `true` while the `AutoGeneratedField` of `DisplayAttribute` is `false`. For the `ID` property, the `Browsable` is `false`, so the `AutoGeneratedField` property has no effect.
