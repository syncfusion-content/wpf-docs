...
# Setting Header in WPF Tabbed MDI Form

Using the `Header` attached property, the user can set the header for each child element. The `Header` accepts any `object`, so the header text can be customized using a `HeaderTemplate`. Use the following code snippet to set the header for a WPF Tabbed MDI Form element.
...
The header of a WPF Tabbed MDI Form element can be set by the `SetHeader` static method.
