---
layout: post
title: Image cropping in Syncfusion SfImageEditor for WPF
description: This section describes various ways to select a cropping area and to crop the image with specified bounds using the SfImageEditor control.
platform: wpf
...
An image can be cropped using the toolbar and programmatically.
...
Clicking the `Crop` icon displays a sub toolbar below the main toolbar. After selecting the cropping portion, click `OK` in the sub toolbar to crop the selected portion.
...
* OK - Crops the selected portion.
* Cancel - Removes the cropping handles.

N> On specifying width and height, the cropping area is located from the top-left corner.
...
The following screenshot illustrates the resulting image after performing cropping on the selected area.

![Cropped image](Images/CroppedImage.jpg)

An image can be cropped in rectangle and circle format. By default, the rectangle format is enabled in the toolbar and you can enable the circle format by using the drop-down button in the crop icon.

![Circle cropping toolbar image](Images/CircleCropping_Toolbar_Img.png)

## Programmatic cropping

Cropping can be done programmatically using the following methods in the image editor:
...
The [`ToggleCropping`](https://help.syncfusion.com/cr/wpf/Syncfusion.UI.Xaml.ImageEditor.SfImageEditor.html#Syncfusion_UI_Xaml_ImageEditor_SfImageEditor_ToggleCropping) method selects the cropping area based on the specified parameters.
...
The following method takes the ratio as a parameter to select the cropping area.
...
The following method takes a `Rect` in terms of percentage to select the cropping area.
...
Set the optional parameter of [`ToggleCropping`](https://help.syncfusion.com/cr/wpf/Syncfusion.UI.Xaml.ImageEditor.SfImageEditor.html#Syncfusion_UI_Xaml_ImageEditor_SfImageEditor_ToggleCropping) to `true` as shown in the following code sample to enable the circle or elliptical format.
...
To manually select and crop a location, use the same [`Crop`](https://help.syncfusion.com/cr/wpf/Syncfusion.UI.Xaml.ImageEditor.SfImageEditor.html#Syncfusion_UI_Xaml_ImageEditor_SfImageEditor_Crop_System_Windows_Rect_System_Boolean_) method, but specify the portion to be cropped in terms of a `Rect`, as in the following code.
