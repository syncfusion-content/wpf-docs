...
1. When using this, no need to have custom logic in the business objects for updating the hierarchy so that the code in a business object class/data structure will be reduced.
...
By default, the `UseAutoUpdateHierarchy` property is set to `true` so that this feature will work in all applications by default. If you need to prevent this and want to update the hierarchy by your own logic, then you have to set this property as `false`.
...
## Using Custom Logic in Business Objects

To use your own logic in business objects:
...
public class Task : NotificationObject
{
    ...
    public TimeSpan Duration
    {
        ...
        set
        {
            ...
            duration = value;
            /// The end date is calculated based on the duration. The duration is linked with the start date and end date, so it will affect both based on the changes.
    ...
    public ObservableCollection<Task> ChildCollection
    {
        get
        {
            ...
            {
                this.childCollection = new ObservableCollection<Task>();
                /// The collection of child tasks is hooked to listen and refresh the parent node based on the changes made in child nodes.
        ...
        set
        {
            ...
            if (value.Count > 0)
            {
                ...
                {
                    /// To listen to the changes made in child tasks.
